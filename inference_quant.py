...
def process(
    ...
    bpp = model.apply_condition_compress(control, stream_path, height, width,QS_global)
    cond = {
        "c_latent": [model.apply_condition_decompress(stream_path,QS_global)],
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("--ckpt", default='logs/1_2_1/lightning_logs/version_4/checkpoints/step=9999.ckpt', type=str, help="full checkpoint path")
    parser.add_argument("--config", default='configs/model/diffeic.yaml', type=str, help="model config path")
    
    parser.add_argument("--input", type=str, default='/home/dongnan/SLF/data/image/kodak_512')
    parser.add_argument("--sampler", type=str, default="ddpm", choices=["ddpm", "ddim"])
    parser.add_argument("--steps", default=10, type=int)
    parser.add_argument("--QS_global", default=0.4, type=float)
...
def main() -> None:
    ...
    for file_path in list_image_files(args.input, follow_links=True):
        ...
        preds, bpp = process(
            model, [x], steps=args.steps,QS_global = args.QS_global, sampler=args.sampler,
