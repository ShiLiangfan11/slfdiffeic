...
class CompressionModel(nn.Module):
    ...
    def quant(self, x,training=False, force_detach=False):
        if training or force_detach:
            n = torch.round(x) - x
            n = n.clone().detach()
            return x + n
    ...
    def process_with_mask(self, y, scales, means, mask):
        ...
        y_q_sof = self.quant(y_res,training = True)
        y_hat = y_q_sof + means_hat

        return y_res, y_q,y_q_sof, y_hat, scales_hat
    ...
    def forward_four_part_prior(self, y, common_params,
        ...
        y_res_0, y_q_0,y_q_sof_0, y_hat_0, s_hat_0 = self.process_with_mask(y, scales, means, mask_0)
        ...
        y_res_1, y_q_1,y_q_sof_1, y_hat_1, s_hat_1 = self.process_with_mask(y, scales, means, mask_1)

        y_hat_so_far = y_hat_so_far + y_hat_1
        params = torch.cat((y_hat_so_far, common_params), dim=1)
        scales, means = y_spatial_prior(y_spatial_prior_adaptor_2(params)).chunk(2, 1)
        y_res_2, y_q_2,y_q_sof_2, y_hat_2, s_hat_2 = self.process_with_mask(y, scales, means, mask_2)

        y_hat_so_far = y_hat_so_far + y_hat_2
        params = torch.cat((y_hat_so_far, common_params), dim=1)
        scales, means = y_spatial_prior(y_spatial_prior_adaptor_3(params)).chunk(2, 1)
        y_res_3, y_q_3,y_q_sof_3, y_hat_3, s_hat_3 = self.process_with_mask(y, scales, means, mask_3)

        y_res = (y_res_0 + y_res_1) + (y_res_2 + y_res_3)
        y_q = (y_q_0 + y_q_1) + (y_q_2 + y_q_3)
        y_q_sof = (y_q_sof_0 + y_q_sof_1) + (y_q_sof_2 + y_q_sof_3)
