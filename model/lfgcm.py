...
class Quantstep(nn.Module):
    def __init__(self, N, M):
        ...
        self.hyper_dec = nn.Sequential(
            ...
            deconv(M, M ),
            nn.GELU(),
            deconv(M, M, kernel_size=3, stride=1),
        )
...
class LFGCM(CompressionModel):
    def __init__(self, in_nc, out_nc, enc_mid, N, M, prior_nc, sft_ks, slice_num, slice_ch):
        ...
        self.q_s = Quantstep(N,M)


        self.entropy_bottleneck = EntropyBottleneck(N)
        self.gaussian_conditional = GaussianConditional(None)
        self.y_q_basic = nn.Parameter(torch.ones((1, M, 1, 1)))
    ...
    def forward(self, x, ref, qs_global):
        ...
        #print("hyper_param:",hyper_params)
        q_s_en = self.q_s(z_hat)
        quant_step = LowerBound.apply(q_s_en, 0.5)
        #print("quant_step:",quant_step)
        y = y / quant_step 
        ...
        for idx, y_slice in enumerate(y_slices):
            ...
            if idx == 0:
                ...
                #print("slice_anchor:",slice_anchor)
                #print("scales_anchor:",scales_anchor)
                #print("means_anchor:",means_anchor)
                # Non-anchor
                # local_ctx: [B, H, W, 2 * C]
                ...
                #print("y_slice_likelihoods :",y_slice_likelihoods )
    ...
    def compress(self, x, ref, qs_global):
    ...
    def decompress(self, strings, shape, qs_global):
        torch.backends.cudnn.deterministic = True
        q_g = self.get_curr_y_q(qs_global)
