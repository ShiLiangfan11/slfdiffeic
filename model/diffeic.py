...
class DiffEIC(LatentDiffusion):
    ...
    def __init__(
        ...
        l_bpp_weight_min: float,
        l_bpp_weight_max: float,
        ...
        **kwargs
        ...
        self.l_bpp_weight_min = l_bpp_weight_min
        self.l_bpp_weight_max = l_bpp_weight_max
    ...
    def apply_condition_encoder(self,  x,image, qs_global):
        c_latent, likelihoods, q_likelihoods = self.preprocess_model( x,image, qs_global)
        return c_latent, likelihoods, q_likelihoods
    
    @torch.no_grad()
    def apply_condition_compress(self, control, stream_path, H, W,QS_global):
        ref = self.encode_first_stage(control * 2 - 1).mode() * self.scale_factor
        out = self.preprocess_model.compress( ref,QS_global)
        bit_stream = out['bit_stream']
        with open(stream_path, "wb") as f:  # 使用 "wb" 模式以二进制写入
         written = write_ip(f, True, 0, bit_stream)  # 传递文件对象 f
        bpp = float(written) * 8 / (H * W)
        return bpp

    @torch.no_grad()
    def apply_condition_decompress(self, stream_path,QS_global):
        with Path(stream_path).open("rb") as f:
            strings, shape = read_body(f)
        c_latent = self.preprocess_model.decompress(strings, shape,QS_global)
        return c_latent
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        #global_q = random.uniform(self.l_bpp_weight_min, self.l_bpp_weight_max)
        global_q = 0.4
        lambda_bpp = 2 ** (2.5 * global_q -1)
        c_latent, likelihoods, q_likelihoods = self.apply_condition_encoder(x, control, global_q)
        ...
        return x, dict(c_crossattn=[c], c_latent=[c_latent], bpp=bpp, q_bpp=q_bpp, control=[control]),lambda_bpp
    ...
    def log_images(self, batch, sample_steps=50, bs=2):
        log = dict()
        z, c,_ = self.get_input(batch, self.first_stage_key, bs=bs)
        bpp = c["q_bpp"]
        bpp_img = [f'{bpp.mean().item():2f}']*4  # Use .item() to get the scalar value
    ...
    def p_losses(self, x_start, cond, t,lamada_bpp, noise=None):
        loss_dict = {}
